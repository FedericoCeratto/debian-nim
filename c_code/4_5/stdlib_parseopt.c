...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
;
static NIM_CONST TY211607 TMP1570 = {
...
N_NIMCALL(void, initoptparser_207030)(NimStringDesc* cmdline, optparser207011* Result) {
	...
	LA3: ;
	{
		unsureAsgnRef((void**) (&(*Result).Cmd), copyString(((NimStringDesc*) &TMP1566)));
		{
			NI i_207046;
			NI HEX3Atmp_207049;
			NI res_207052;
...
appendChar(LOC9, 32);
	...
	LA1: ;
	(*Result).Kind = ((NU8) 0);
	unsureAsgnRef((void**) (&(*Result).Key), copyString(((NimStringDesc*) &TMP1568)));
	unsureAsgnRef((void**) (&(*Result).Val), copyString(((NimStringDesc*) &TMP1568)));
}
...
N_NIMCALL(void, handleshortoption_207309)(optparser207011* p) {
	...
	}
	{
		...
		i = parseword_207204((*p).Cmd, i, (&(*p).Val), TMP1569);
	}
	LA5: ;
	{
...
N_NIMCALL(void, nponext)(optparser207011* p) {
	...
	case 45:
	{
		i += 1;
		{
			...
			(*p).Kind = ((NU8) 2);
			i += 1;
			i = parseword_207204((*p).Cmd, i, (&(*p).Key), TMP1570);
			{
				while (1) {
			...
			}
			{
				...
				(*p).Pos = parseword_207204((*p).Cmd, i, (&(*p).Val), TMP1569);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).Kind = ((NU8) 1);
		(*p).Pos = parseword_207204((*p).Cmd, i, (&(*p).Key), TMP1569);
	}
	break;
	}
	BeforeRet: ;
...
static TNimNode* TMP1562[6];
static TNimNode* TMP1563[4];
NI TMP1565;
static char* NIM_CONST TMP1564[4] = {
"cmdEnd", 
"cmdArgument", 
"cmdLongOption", 
"cmdShortOption"};
...
NTI207009.base = 0;
NTI207009.flags = 3;
for (TMP1565 = 0; TMP1565 < 4; TMP1565++) {
TMP706[TMP1565+5].kind = 1;
TMP706[TMP1565+5].offset = TMP1565;
TMP706[TMP1565+5].name = TMP1564[TMP1565];
TMP1563[TMP1565] = &TMP706[TMP1565+5];
}
TMP706[9].len = 4; TMP706[9].kind = 2; TMP706[9].sons = &TMP1563[0];
...
TMP706[11].name = "val";
TMP706[0].len = 6; TMP706[0].kind = 2; TMP706[0].sons = &TMP1562[0];
