...
N_NIMCALL(void, handleshortoption_175291)(Optparser175011* p) {
	...
	}
	{
		...
		i = parseword_175204((*p).cmd, i, (&(*p).val), TMP1538);
	}
	LA5: ;
	{
...
{	i = (*p).pos;
	...
	case 45:
	{
		i += ((NI) 1);
		{
			...
			(*p).kind = ((NU8) 2);
			i += ((NI) 1);
			i = parseword_175204((*p).cmd, i, (&(*p).key), TMP1539);
			{
				while (1) {
			...
			}
			{
				...
				(*p).pos = parseword_175204((*p).cmd, i, (&(*p).val), TMP1538);
			}
			goto LA15;
			LA17: ;
			{
	...
	{
		(*p).kind = ((NU8) 1);
		(*p).pos = parseword_175204((*p).cmd, i, (&(*p).key), TMP1538);
	}
	break;
	}
	}BeforeRet: ;
