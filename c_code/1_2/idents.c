/* Generated by Nim Compiler v0.10.2 */
/*   (c) 2014 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 64
#include "nimbase.h"
typedef struct tident201021 tident201021;
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct tidobj201015 tidobj201015;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct tcell44733 tcell44733;
typedef struct tcellseq44749 tcellseq44749;
typedef struct tgcheap46816 tgcheap46816;
typedef struct tcellset44745 tcellset44745;
typedef struct tpagedesc44741 tpagedesc44741;
typedef struct tmemregion27010 tmemregion27010;
typedef struct tsmallchunk26240 tsmallchunk26240;
typedef struct tllchunk27004 tllchunk27004;
typedef struct tbigchunk26242 tbigchunk26242;
typedef struct tintset26217 tintset26217;
typedef struct ttrunk26213 ttrunk26213;
typedef struct tavlnode27008 tavlnode27008;
typedef struct tgcstat46814 tgcstat46814;
typedef struct tbasechunk26238 tbasechunk26238;
typedef struct tfreecell26230 tfreecell26230;
typedef tident201021* TY201034[8192];
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
typedef N_NIMCALL_PTR(void, TY3089) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3094) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3089 marker;
TY3094 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  tidobj201015  {
  TNimObject Sup;
NI Id;
};
struct  tident201021  {
  tidobj201015 Sup;
NimStringDesc* S;
tident201021* Next;
NI H;
};
struct  tcell44733  {
NI Refcount;
TNimType* Typ;
};
struct  tcellseq44749  {
NI Len;
NI Cap;
tcell44733** D;
};
struct  tcellset44745  {
NI Counter;
NI Max;
tpagedesc44741* Head;
tpagedesc44741** Data;
};
typedef tsmallchunk26240* TY27022[512];
typedef ttrunk26213* ttrunkbuckets26215[256];
struct  tintset26217  {
ttrunkbuckets26215 Data;
};
struct  tmemregion27010  {
NI Minlargeobj;
NI Maxlargeobj;
TY27022 Freesmallchunks;
tllchunk27004* Llmem;
NI Currmem;
NI Maxmem;
NI Freemem;
NI Lastsize;
tbigchunk26242* Freechunkslist;
tintset26217 Chunkstarts;
tavlnode27008* Root;
tavlnode27008* Deleted;
tavlnode27008* Last;
tavlnode27008* Freeavlnodes;
};
struct  tgcstat46814  {
NI Stackscans;
NI Cyclecollections;
NI Maxthreshold;
NI Maxstacksize;
NI Maxstackcells;
NI Cycletablesize;
NI64 Maxpause;
};
struct  tgcheap46816  {
void* Stackbottom;
NI Cyclethreshold;
tcellseq44749 Zct;
tcellseq44749 Decstack;
tcellset44745 Cycleroots;
tcellseq44749 Tempstack;
NI Recgclock;
tmemregion27010 Region;
tgcstat46814 Stat;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
typedef NI TY26220[8];
struct  tpagedesc44741  {
tpagedesc44741* Next;
NI Key;
TY26220 Bits;
};
struct  tbasechunk26238  {
NI Prevsize;
NI Size;
NIM_BOOL Used;
};
struct  tsmallchunk26240  {
  tbasechunk26238 Sup;
tsmallchunk26240* Next;
tsmallchunk26240* Prev;
tfreecell26230* Freelist;
NI Free;
NI Acc;
NF Data;
};
struct  tllchunk27004  {
NI Size;
NI Acc;
tllchunk27004* Next;
};
struct  tbigchunk26242  {
  tbasechunk26238 Sup;
tbigchunk26242* Next;
tbigchunk26242* Prev;
NI Align;
NF Data;
};
struct  ttrunk26213  {
ttrunk26213* Next;
NI Key;
TY26220 Bits;
};
typedef tavlnode27008* TY27014[2];
struct  tavlnode27008  {
TY27014 Link;
NI Key;
NI Upperbound;
NI Level;
};
struct  tfreecell26230  {
tfreecell26230* Next;
NI Zerofield;
};
N_NIMCALL(tident201021*, getident_201472)(NimStringDesc* identifier);
N_NIMCALL(tident201021*, getident_201268)(NCSTRING identifier, NI length, NI h);
N_NIMCALL(NI, cmpexact_201200)(NCSTRING a, NCSTRING b, NI blen);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
static N_INLINE(tcell44733*, usrtocell_48446)(void* usr);
static N_INLINE(void, rtladdzct_50004)(tcell44733* c);
N_NOINLINE(void, addzct_48417)(tcellseq44749* s, tcell44733* c);
N_NIMCALL(NI, cmpignorestyle_201044)(NCSTRING a, NCSTRING b, NI blen);
N_NIMCALL(void, nimGCvisit)(void* d, NI op);
N_NIMCALL(void, TMP454)(void* p, NI op);
N_NIMCALL(void*, newObj)(TNimType* typ, NI size);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NI, hashignorestyle_130885)(NimStringDesc* x);
N_NIMCALL(void, TMP839)(void* p, NI op);
STRING_LITERAL(TMP455, ":anonymous", 10);
STRING_LITERAL(TMP456, ":delegator", 10);
NIM_BOOL firstchariscs_201029;
TY201034 buckets_201039;
NI wordcounter_201264;
tident201021* idanon_201504;
extern tgcheap46816 gch_46844;
extern TNimType NTI3211; /* RootObj */
TNimType NTI201015; /* TIdObj */
extern TNimType NTI108; /* int */
TNimType NTI201021; /* TIdent */
extern TNimType NTI149; /* string */
extern TNimType NTI130021; /* THash */
TNimType NTI201019; /* PIdent */
tident201021* iddelegator_201559;
TNimType NTI201017; /* PIdObj */

N_NIMCALL(NI, cmpexact_201200)(NCSTRING a, NCSTRING b, NI blen) {
	NI result;
	NI i;
	NI j;
	result = 0;
	i = 0;
	j = 0;
	result = 1;
	{
		while (1) {
			NIM_CHAR aa;
			NIM_CHAR bb;
			if (!(j < blen)) goto LA2;
			aa = a[i];
			bb = b[j];
			result = (NI64)(((NI) (((NU8)(aa)))) - ((NI) (((NU8)(bb)))));
			{
				NIM_BOOL LOC5;
				LOC5 = 0;
				LOC5 = !((result == 0));
				if (LOC5) goto LA6;
				LOC5 = ((NU8)(aa) == (NU8)(0));
				LA6: ;
				if (!LOC5) goto LA7;
				goto LA1;
			}
			LA7: ;
			i += 1;
			j += 1;
		} LA2: ;
	} LA1: ;
	{
		if (!(result == 0)) goto LA11;
		{
			if (!!(((NU8)(a[i]) == (NU8)(0)))) goto LA15;
			result = 1;
		}
		LA15: ;
	}
	LA11: ;
	return result;
}

static N_INLINE(tcell44733*, usrtocell_48446)(void* usr) {
	tcell44733* result;
	result = 0;
	result = ((tcell44733*) ((NI)((NU64)(((NI) (usr))) - (NU64)(((NI)sizeof(tcell44733))))));
	return result;
}

static N_INLINE(void, rtladdzct_50004)(tcell44733* c) {
	addzct_48417((&gch_46844.Zct), c);
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	{
		tcell44733* c;
		if (!!((src == NIM_NIL))) goto LA3;
		c = usrtocell_48446(src);
		(*c).Refcount += 8;
	}
	LA3: ;
	{
		tcell44733* c;
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		c = usrtocell_48446((*dest));
		{
			(*c).Refcount -= 8;
			if (!((NU64)((*c).Refcount) < (NU64)(8))) goto LA11;
			rtladdzct_50004(c);
		}
		LA11: ;
	}
	LA7: ;
	(*dest) = src;
}

N_NIMCALL(NI, cmpignorestyle_201044)(NCSTRING a, NCSTRING b, NI blen) {
	NI result;
	NI i;
	NI j;
	result = 0;
	{
		if (!firstchariscs_201029) goto LA3;
		{
			if (!!(((NU8)(a[0]) == (NU8)(b[0])))) goto LA7;
			result = 1;
			goto BeforeRet;
		}
		LA7: ;
	}
	LA3: ;
	i = 0;
	j = 0;
	result = 1;
	{
		while (1) {
			NIM_CHAR aa;
			NIM_CHAR bb;
			if (!(j < blen)) goto LA10;
			{
				while (1) {
					if (!((NU8)(a[i]) == (NU8)(95))) goto LA12;
					i += 1;
				} LA12: ;
			}
			{
				while (1) {
					if (!((NU8)(b[j]) == (NU8)(95))) goto LA14;
					j += 1;
				} LA14: ;
			}
			aa = a[i];
			bb = b[j];
			{
				NIM_BOOL LOC17;
				LOC17 = 0;
				LOC17 = ((NU8)(65) <= (NU8)(aa));
				if (!(LOC17)) goto LA18;
				LOC17 = ((NU8)(aa) <= (NU8)(90));
				LA18: ;
				if (!LOC17) goto LA19;
				aa = ((NIM_CHAR) (((NI) ((NI64)(((NI) (((NU8)(aa)))) + 32)))));
			}
			LA19: ;
			{
				NIM_BOOL LOC23;
				LOC23 = 0;
				LOC23 = ((NU8)(65) <= (NU8)(bb));
				if (!(LOC23)) goto LA24;
				LOC23 = ((NU8)(bb) <= (NU8)(90));
				LA24: ;
				if (!LOC23) goto LA25;
				bb = ((NIM_CHAR) (((NI) ((NI64)(((NI) (((NU8)(bb)))) + 32)))));
			}
			LA25: ;
			result = (NI64)(((NI) (((NU8)(aa)))) - ((NI) (((NU8)(bb)))));
			{
				NIM_BOOL LOC29;
				LOC29 = 0;
				LOC29 = !((result == 0));
				if (LOC29) goto LA30;
				LOC29 = ((NU8)(aa) == (NU8)(0));
				LA30: ;
				if (!LOC29) goto LA31;
				goto LA9;
			}
			LA31: ;
			i += 1;
			j += 1;
		} LA10: ;
	} LA9: ;
	{
		if (!(result == 0)) goto LA35;
		{
			if (!!(((NU8)(a[i]) == (NU8)(0)))) goto LA39;
			result = 1;
		}
		LA39: ;
	}
	LA35: ;
	BeforeRet: ;
	return result;
}
N_NIMCALL(void, TMP454)(void* p, NI op) {
	tident201021* a;
	a = (tident201021*)p;
	nimGCvisit((void*)(*a).S, op);
	nimGCvisit((void*)(*a).Next, op);
}

N_NIMCALL(tident201021*, getident_201268)(NCSTRING identifier, NI length, NI h) {
	tident201021* result;
	NI idx;
	tident201021* last;
	NI id;
	result = 0;
	idx = (NI)(h & 8191);
	result = buckets_201039[(idx)- 0];
	last = NIM_NIL;
	id = 0;
	{
		while (1) {
			if (!!((result == NIM_NIL))) goto LA2;
			{
				NI LOC5;
				LOC5 = 0;
				LOC5 = cmpexact_201200((*result).S->data, identifier, length);
				if (!(LOC5 == 0)) goto LA6;
				{
					if (!!((last == NIM_NIL))) goto LA10;
					asgnRefNoCycle((void**) (&(*last).Next), (*result).Next);
					asgnRefNoCycle((void**) (&(*result).Next), buckets_201039[(idx)- 0]);
					asgnRefNoCycle((void**) (&buckets_201039[(idx)- 0]), result);
				}
				LA10: ;
				goto BeforeRet;
			}
			goto LA3;
			LA6: ;
			{
				NI LOC13;
				LOC13 = 0;
				LOC13 = cmpignorestyle_201044((*result).S->data, identifier, length);
				if (!(LOC13 == 0)) goto LA14;
				id = (*result).Sup.Id;
			}
			goto LA3;
			LA14: ;
			LA3: ;
			last = result;
			result = (*result).Next;
		} LA2: ;
	}
	result = (tident201021*) newObj((&NTI201019), sizeof(tident201021));
	(*result).Sup.Sup.m_type = (&NTI201021);
	(*result).H = h;
	asgnRefNoCycle((void**) (&(*result).S), mnewString(length));
	{
		NI i_201429;
		NI HEX3Atmp_201446;
		NI res_201449;
		i_201429 = 0;
		HEX3Atmp_201446 = 0;
		HEX3Atmp_201446 = (NI64)(length - 1);
		res_201449 = 0;
		{
			while (1) {
				if (!(res_201449 <= HEX3Atmp_201446)) goto LA18;
				i_201429 = res_201449;
				(*result).S->data[i_201429] = identifier[i_201429];
				res_201449 += 1;
			} LA18: ;
		}
	}
	asgnRefNoCycle((void**) (&(*result).Next), buckets_201039[(idx)- 0]);
	asgnRefNoCycle((void**) (&buckets_201039[(idx)- 0]), result);
	{
		if (!(id == 0)) goto LA21;
		wordcounter_201264 += 1;
		(*result).Sup.Id = ((NI64)-(wordcounter_201264));
	}
	goto LA19;
	LA21: ;
	{
		(*result).Sup.Id = id;
	}
	LA19: ;
	BeforeRet: ;
	return result;
}

N_NIMCALL(tident201021*, getident_201472)(NimStringDesc* identifier) {
	tident201021* result;
	NI LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = hashignorestyle_130885(identifier);
	result = getident_201268(identifier->data, identifier->Sup.len, LOC1);
	return result;
}

N_NIMCALL(tident201021*, getident_201482)(NimStringDesc* identifier, NI h) {
	tident201021* result;
	result = 0;
	result = getident_201268(identifier->data, identifier->Sup.len, h);
	return result;
}
N_NIMCALL(void, TMP839)(void* p, NI op) {
	tidobj201015* a;
	a = (tidobj201015*)p;
}

N_NIMCALL(NIM_BOOL, identeq_201493)(tident201021* id, NimStringDesc* name) {
	NIM_BOOL result;
	tident201021* LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = getident_201472(name);
	result = ((*id).Sup.Id == (*LOC1).Sup.Id);
	return result;
}
NIM_EXTERNC N_NOINLINE(void, HEX00_identsInit)(void) {
	wordcounter_201264 = 1;
	asgnRefNoCycle((void**) (&idanon_201504), getident_201472(((NimStringDesc*) &TMP455)));
	asgnRefNoCycle((void**) (&iddelegator_201559), getident_201472(((NimStringDesc*) &TMP456)));
}

NIM_EXTERNC N_NOINLINE(void, HEX00_identsDatInit)(void) {
static TNimNode* TMP453[3];
static TNimNode TMP451[5];
NTI201015.size = sizeof(tidobj201015);
NTI201015.kind = 17;
NTI201015.base = (&NTI3211);
NTI201015.flags = 1;
TMP451[0].kind = 1;
TMP451[0].offset = offsetof(tidobj201015, Id);
TMP451[0].typ = (&NTI108);
TMP451[0].name = "id";
NTI201015.node = &TMP451[0];
NTI201021.size = sizeof(tident201021);
NTI201021.kind = 17;
NTI201021.base = (&NTI201015);
NTI201021.flags = 2;
TMP453[0] = &TMP451[2];
TMP451[2].kind = 1;
TMP451[2].offset = offsetof(tident201021, S);
TMP451[2].typ = (&NTI149);
TMP451[2].name = "s";
TMP453[1] = &TMP451[3];
TMP451[3].kind = 1;
TMP451[3].offset = offsetof(tident201021, Next);
TMP451[3].typ = (&NTI201019);
TMP451[3].name = "next";
TMP453[2] = &TMP451[4];
TMP451[4].kind = 1;
TMP451[4].offset = offsetof(tident201021, H);
TMP451[4].typ = (&NTI130021);
TMP451[4].name = "h";
TMP451[1].len = 3; TMP451[1].kind = 2; TMP451[1].sons = &TMP453[0];
NTI201021.node = &TMP451[1];
NTI201019.size = sizeof(tident201021*);
NTI201019.kind = 22;
NTI201019.base = (&NTI201021);
NTI201019.flags = 2;
NTI201019.marker = TMP454;
NTI201017.size = sizeof(tidobj201015*);
NTI201017.kind = 22;
NTI201017.base = (&NTI201015);
NTI201017.marker = TMP839;
}

