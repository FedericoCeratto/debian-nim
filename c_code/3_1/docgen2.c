...
N_NIMCALL(tnode225816*, close_609018)(tpasscontext286005* p, tnode225816* n) {
	...
	usewarning = !((((*(*g).Module).Flags &(1<<((((NU8) 12))&31)))!=0));
	{
		...
		writeoutput_403920((*g).Doc, LOC7, ((NimStringDesc*) &TMP4746), usewarning);
		pushSafePoint(&TMP4747);
		TMP4747.status = _setjmp(TMP4747.context);
		if (TMP4747.status == 0) {
		...
		if (TMP4747.status != 0) reraiseException();
	}
	LA5: ;
	return result;
}
